...
class latticePlanner:
    ...
    def checkObject(self, ref_path, object_data):
        ...
        for obstacle in object_data.obstacle_list:
            for path in ref_path.poses:  
                ...
                if dis < 2.1: # 장애물의 좌표값이 지역 경로 상의 좌표값과의 직선거리가 2.35 미만일때 충돌이라 판단.
                    is_crash = True
                    break
    ...
    def collision_check(self, object_data, out_path):
        ...
        selected_lane = -1        
        lane_weight = [2, 1, 1, 2] #reference path 
        ...
        if len(ref_path.poses) > look_distance :  
            ...
            for end_point in local_lattice_points :
                ...
                for i in range(0,len(y)) :
                    ...
                    # 추가 : 생성된 경로 후륜 방향으로 당기기.
                    rear_offset = 5.0        
                    global_result[0][0] -= rear_offset * cos(theta)   # x방향(차량 진행축) -1.8 m
                    global_result[1][0] -= rear_offset * sin(theta)   # y방향(수직축)      -1.8 m
            ...
            for i in range(look_distance*2,add_point_size): # 수정 : +100
